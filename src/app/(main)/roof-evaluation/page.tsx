...
export default function RoofEvaluation() {
  const pinnedRef = useRef<HTMLDivElement>(null);
  const scrollRef = useRef<HTMLDivElement>(null);

  const service = SERVICES.find((s) => s.href === "/roof-evaluation");
  const timeline1 = service?.timelines?.[0];
  const heroTitle =
    service?.title || "Roof Evaluation – Protect Your Home From the Top Down";
  const heroDescription =
    service?.description || "Professional roof evaluation and repair services";
  ...
  return (
    <>
      <ServiceStructuredData
        ...
        path="/roof-evaluation"
      />
      <BreadcrumbStructuredData
        items={[
          { name: "Home", url: "https://brestorations.com" },
          {
            name: service?.label || "Roof Evaluation",
      ...
      <main>
        ...
        >
          <div className="absolute top-0 left-0 -z-1 h-full w-full">
            <OptimizedVideo
              src={HERO_VIDEOS.roofEvaluation.src}
              poster={HERO_VIDEOS.roofEvaluation.poster}
              alt={HERO_VIDEOS.roofEvaluation.alt}
          ...
          <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 md:grid-cols-2 md:gap-12 lg:grid-cols-12 lg:gap-20">
            ...
            >
              ...
              <h2 className="mx-auto mb-8 w-full text-left text-3xl leading-8 font-semibold text-balance">
                {timeline1?.title || "How Our Roof Evaluation Works"}
              </h2>
